...
class RobotControlNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('movel.x_min', -0.4)
        self.declare_parameter('movel.x_max', 0.1)
        self.declare_parameter('movel.y_min', -0.3)
        self.declare_parameter('movel.y_max', 0.3)
        self.declare_parameter('movel.z_min', 0.17)
        self.declare_parameter('movel.z_max', 0.35)
    ...
    def send_command(self, data: dict):
        ...
        self.urx_command_publisher.publish(String(data=json.dumps(data)))
        ...
        if data and "type" in data: 
            if data["type"] == "movel": # movel
                ...
                if (self.limits.z_min <= coords[2] <= self.limits.z_max) and (self.limits.x_min <= coords[0] <= self.limits.x_max) and (self.limits.y_min <= coords[1] <= self.limits.y_max): 
                    #iself.send_command(f"{}")
                    #self.urx_command_publisher.publish(String(data=f'''{{'type': 'movel', 'data': {data["data"]}, 'velocity': {self.limits.vel}, 'acceleration': {self.limits.acc}}}'''))
                    self.send_command({"type": "movel", "data": data["data"], "velocity": self.limits.vel, "acceleration": self.limits.acc})
                    #self.get_logger().info("Sent to urx_node")
            
                else:
                    self.get_logger().warning(f"Can't MOVE LIMITS: {self.limits} with pose: {coords}")
